...
/**
 * Update Piresa's role to ensure CRM access
 */
...
async function updatePiresaRole() {
  try {
    console.log('🔧 UPDATING PIRESA ROLE FOR CRM ACCESS')
    console.log('======================================\n')

    const email = 'piresa@universalbeautystudio.com'
    const targetRole = 'staff' // CRM requires: owner, staff, manager, or director
    ...
    if (hasCrmAccess) {
      console.log('✅ Piresa already has CRM access!')
    ...
    if (nowHasAccess) {
      ...
      console.log('   1. Piresa needs to log out and log back in')
